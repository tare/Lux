...
def generate_output_files(data_file,prior_file,control_data_file,control_prior_file,prefix):
  ...
  if prior.shape[0] != 1 :
    sys.exit('error: there should be exactly one line in %s'%(prior_file))
  ...
  g = numpy.random.gamma(g_a,1.0/g_b)
  f_v = numpy.random.gamma(g_a,1.0/g_b)
  theta = ','.join(numpy.array([map(str,numpy.random.dirichlet(row)) for row in numpy.tile(numpy.tile(prior,(N,1)),(R,1))]).flatten(1))
  theta_control = ','.join(numpy.array([map(str,numpy.random.dirichlet(row)) for row in numpy.tile(control_prior,(R,1))]).flatten(1))
  mu = ','.join(map(str,numpy.random.dirichlet(prior[0])))
  nu = ','.join(numpy.array([map(str,numpy.random.dirichlet(prior[0])) for row in range(0,N)]).flatten(1))
  ...
  with open(prefix+'_init.R','w') as f:
    ...
    f.write("nu <- structure(c(%s), .Dim=c(%d,3))\n" % (nu,N))
    f.write("mu <- c(%s)\n" % (mu))
